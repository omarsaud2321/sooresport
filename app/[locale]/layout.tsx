            ...
            <header className="sticky top-0 z-10 border-b border-border bg-bg/80 backdrop-blur">
              <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
                ...
                <nav className="flex items-center gap-4 text-sm text-muted">
                  ...
                  <Link href={`/admin`} className="text-muted hover:text-text">Admin</Link>
                  <div className="flex items-center gap-2 rounded border border-border bg-card px-2 py-1">
                    <Link href={`/ar`} className={locale === "ar" ? "text-text" : ""}>AR</Link>
                    <span className="text-border">|</span>
                    <Link href={`/en`} className={locale === "en" ? "text-text" : ""}>EN</Link>
                  </div>
                </nav>
              </div>
            </header>
